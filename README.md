...
WindowOrchestrator is a macOS menu bar app for arranging and orchestrating application windows. It provides automatic tiling groups, focus groups, focused-window resizing, and global hotkeys for common window workflows.
...
- Accessibility permission for WindowOrchestrator, required for moving and inspecting windows.

## Build

Open `WindowOrchestrator.xcodeproj` in Xcode and build the `WindowOrchestrator` scheme.

From the command line:

```sh
xcodebuild -project WindowOrchestrator.xcodeproj -scheme WindowOrchestrator -configuration Debug build
```

If your active developer directory points at Command Line Tools instead of Xcode, select Xcode first or set `DEVELOPER_DIR`:

```sh
DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer xcodebuild -project WindowOrchestrator.xcodeproj -scheme WindowOrchestrator -configuration Debug build
```

## Open Source

WindowOrchestrator is released under the MIT License. See `LICENSE`.
